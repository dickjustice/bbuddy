...
VERB_SILENT=0;VERB_TERSE=1;VERB_NORMAL=2;VERB_VERBOSE=3;VERB_DEBUG=4;

#======================================================================
# various ansi color codes
RED     = "\033[1;31m"; YELLOW  = "\033[0;31m";MAGENTA = "\033[1;35m";  BLUE    = "\033[1;34m"
CYAN    = "\033[1;36m"; GREEN   = "\033[0;32m"; RESET   = "\033[0;0m"; BOLD    = "\033[;1m"
YELLOW="\033[0;33m";
...
def dump_db_row( row, cnt, start='', printrownum=True, width=80, verb=VERB_NORMAL, longlinehandle='wrap' ):
   if verb>=VERB_DEBUG:
      print( "raw row:", row )
   numcols = len(row);
...
def dump_db_table( c, table_name, start='', width=80, names_of_columns_to_get=[], 
   ...
   try:
      ...
      err_msg( "A db access error occurred")
...
def db_dump( fn_db, start='', top_lev_only=False, id_table_name="_all_tables_", verb=VERB_NORMAL,
            specified_column_names=[], max_table_lines=0, longlinehandle='wrap', width=80 ):
   print( "dump_db:" )
   ...
   if id_table_num >= len(alltables) :
      print( RED+"Error"+RESET+": ", end='' )
      print( "Table number (%d) too big. must be less than %d !" % (id_table_num,len(alltables) ) )
